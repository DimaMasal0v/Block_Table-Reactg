...
interface CFBlockProps {
  recentWorks: RecentWorks;
}

const CFBlock: React.FC<CFBlockProps> = ({ recentWorks }) => {
  const { left, right } = recentWorks;
  const { title, subtitle, images: leftImages, projects: leftProjects, button: leftButton } = left;
  const { images: rightImages, projects: rightProjects, button: rightButton } = right;
    ...
    <section className="relative">
      <div className="grid sm:grid-cols-2 grid-cols-none sm:grid-rows-1 grid-rows-none">
        <div id="container-grid">
          ...
          {leftProjects.map((project, index) => (
            <React.Fragment key={index}>
              ...
              <span className="text-lg font-light text-neutral-400" dangerouslySetInnerHTML={{ __html: project.description }}></span>
            </React.Fragment>
          ))}
          <br />
          <button id={leftButton.id} className="button-contact xl:mt-24 sm:mt-8 text-sm sm:inline-block hidden">
        ...
        <div id="recent-works-right">
          ...
          <br />
          <button id={rightButton.id} className="button-contact xl:mt-24 sm:mt-8 text-sm sm:hidden mt-10 ml-5">
